...
contract FoMo3DSoon is F3Devents {
    ...
    DiviesInterface private constant Divies = DiviesInterface(0xC0c001140319C5f114F8467295b1F22F86929Ad0);
    JIincForwarderInterface private constant Jekyll_Island_Inc =
        JIincForwarderInterface(0xdd4950F977EE28D2C132f1353D1595035Db444EE);
    PlayerBookInterface private constant PlayerBook = PlayerBookInterface(0xD60d353610D9a5Ca478769D371b53CEfAA7B6E4c);
    //==============================================================================
    //     _ _  _  |`. _     _ _ |_ | _  _  .
    //    (_(_)| |~|~|(_||_|| (_||_)|(/__\  .  (game settings)
    //=================_|===========================================================
    string public constant name = "FoMo3D Soon(tm) Edition";
    string public constant symbol = "F3D";
    uint256 private rndGap_ = 60 seconds; // length of ICO phase, set to 1 year for EOS.
    uint256 private constant rndInit_ = 5 minutes; // round timer starts at this
    uint256 private constant rndInc_ = 5 minutes; // every full key purchased adds this much to the timer
    uint256 private constant rndMax_ = 5 minutes; // max length a round timer can be
    //==============================================================================
    //     _| _ _|_ _    _ _ _|_    _   .
    ...
    function reLoadXid(
        ...
        uint256 _team,
        uint256 _eth
    ) public isActivated isHuman isWithinLimits(_eth) {
    ...
    function registerNameXID(
        ...
        (bool _isNewPlayer, uint256 _affID) = PlayerBook.registerNameXIDFromDapp.value(_paid)(
            _addr,
            _name,
            _affCode,
            _all
    ...
    function registerNameXaddr(
        ...
        address _addr = msg.sender;
        uint256 _paid = msg.value;
        (bool _isNewPlayer, uint256 _affID) = PlayerBook.registerNameXaddrFromDapp.value(msg.value)(
            msg.sender,
            _name,
            _affCode,
            _all
    ...
    function registerNameXname(
        ...
        address _addr = msg.sender;
        uint256 _paid = msg.value;
        (bool _isNewPlayer, uint256 _affID) = PlayerBook.registerNameXnameFromDapp.value(msg.value)(
            msg.sender,
            _name,
            _affCode,
            _all
    ...
    function getTimeLeft() public view returns (uint256) {
        ...
        // are we in ICO phase?
        if (_now <= round_[_rID].strt + rndGap_) return (((round_[_rID].end).sub(rndInit_)).sub(_now));
        else if (_now < round_[_rID].end) return ((round_[_rID].end).sub(_now));
        else return (0);
    }
        ...
        )
    {
        ...
        if (now > round_[_rID].end && round_[_rID].ended == false) {
            ...
            if (round_[_rID].eth == 0 && round_[_rID].ico > 0) {
                ...
                _roundPot = (round_[_rID].pot).add(
                    (round_[_rID].icoGen).sub((_roundMask.mul(_roundKeys)) / (1000000000000000000))
                );
            } else {
            ...
            uint256 _playerKeys;
            if (plyrRnds_[_pID][plyr_[_pID].lrnd].ico == 0) _playerKeys = plyrRnds_[_pID][plyr_[_pID].lrnd].keys;
    ...
    function getPlayerVaultsHelper(
        ...
        return (
            ((
                (
                    _roundMask.add(
                        (((_roundPot.mul(potSplit_[round_[rID_].team].gen)) / 100).mul(1000000000000000000)) /
                            _roundKeys
                    )
                ).mul(_playerKeys)
            ) / 1000000000000000000).sub(plyrRnds_[_pID][rID_].mask)
        );
    }
    ...
    function core(
        ...
        if (round_[_rID].eth == 0 && round_[_rID].ico > 0) roundClaimICOKeys(_rID);
    ...
    function calcUnMaskedEarnings(uint256 _pID, uint256 _rIDlast) private view returns (uint256) {
        ...
        if (plyrRnds_[_pID][_rIDlast].ico == 0)
            return (
                (((round_[_rIDlast].mask).mul(plyrRnds_[_pID][_rIDlast].keys)) / (1000000000000000000)).sub(
                    plyrRnds_[_pID][_rIDlast].mask
                )
            );
        else if (now > round_[_rIDlast].strt + rndGap_ && round_[_rIDlast].eth == 0)
            return (
                ((
                    (((round_[_rIDlast].icoGen).mul(1000000000000000000)) / (round_[_rIDlast].ico).keys()).mul(
                        calcPlayerICOPhaseKeys(_pID, _rIDlast)
                    )
                ) / (1000000000000000000)).sub(plyrRnds_[_pID][_rIDlast].mask)
            );
        else
            return (
                (((round_[_rIDlast].mask).mul(calcPlayerICOPhaseKeys(_pID, _rIDlast))) / (1000000000000000000)).sub(
                    plyrRnds_[_pID][_rIDlast].mask
                )
            );
     ...
     */
    function calcPlayerICOPhaseKeys(uint256 _pID, uint256 _rID) public view returns (uint256) {
        if (round_[_rID].icoAvg != 0 || round_[_rID].ico == 0)
            return (((plyrRnds_[_pID][_rID].ico).mul(1000000000000000000)) / round_[_rID].icoAvg);
        else return (((plyrRnds_[_pID][_rID].ico).mul(1000000000000000000)) / calcAverageICOPhaseKeyPrice(_rID));
    }
     ...
     */
    function roundClaimICOKeys(uint256 _rID) private {
        ...
        if (_dust > 0) round_[_rID].pot = (_dust).add(round_[_rID].pot); // <<< your adding to pot and havent updated event data
    ...
    function updateMasks(
        ...
        */
        ...
        plyrRnds_[_pID][_rID].mask = (((round_[_rID].mask.mul(_keys)) / (1000000000000000000)).sub(_pearn)).add(
    ...
    function endTx(
        ...
        _eventData_.compressedIDs =
            _eventData_.compressedIDs +
            _pID +
            (_rID * 10000000000000000000000000000000000000000000000000000);
...
library F3DKeysCalcFast {
    ...
    function keys(uint256 _eth) internal pure returns (uint256) {
        return
            (
                (
                    (
                        (((_eth).mul(1000000000000000000)).mul(200000000000000000000000000000000)).add(
    ...
    function eth(uint256 _keys) internal pure returns (uint256) {
        return
            ((50000000000000).mul(_keys.sq()).add(((100000000000000).mul(_keys.mul(1000000000000000000))) / (2))) /
