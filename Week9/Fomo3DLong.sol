...
contract FoMo3Dlong is modularLong {
    ...
    DiviesInterface private constant Divies = DiviesInterface(0xc7029Ed9EBa97A096e72607f4340c34049C7AF48);
    ...
    F3DexternalSettingsInterface private constant extSettings =
        F3DexternalSettingsInterface(0x32967D6c142c2F38AB39235994e2DDF11c37d590);
    //==============================================================================
    //     _ _  _  |`. _     _ _ |_ | _  _  .
    //    (_(_)| |~|~|(_||_|| (_||_)|(/__\  .  (game settings)
    //=================_|===========================================================
    string public constant name = "FoMo3D Long Official";
    ...
    function getBuyPrice() public view returns (uint256) {
        ...
        else return (75000000000000); // init
    }
    ...
    function getTimeLeft() public view returns (uint256) {
        ...
        if (_now < round_[_rID].end)
            if (_now > round_[_rID].strt + rndGap_) return ((round_[_rID].end).sub(_now));
            ...
            ((
                (
                    (round_[_rID].mask).add(
                        ((((round_[_rID].pot).mul(potSplit_[round_[_rID].team].gen)) / 100).mul(1000000000000000000)) /
                            (round_[_rID].keys)
                    )
...
library F3DKeysCalcLong {
    ...
    function keys(uint256 _eth) internal pure returns (uint256) {
        return
            (
                (
                    (
                        (((_eth).mul(1000000000000000000)).mul(312500000000000000000000000)).add(
    ...
    function eth(uint256 _keys) internal pure returns (uint256) {
        return
            ((78125000).mul(_keys.sq()).add(((149999843750000).mul(_keys.mul(1000000000000000000))) / (2))) /
